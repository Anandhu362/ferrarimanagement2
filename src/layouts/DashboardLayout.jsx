...
export default function DashboardLayout() {
  ...
  // 1. Finance Items (Now includes Sales Entry)
  const financeItems = [
  ...
  // 2. Ops Items (Sales Entry removed)
  const opsItems = [
    { name: 'Delivery Form Gen', path: '/delivery-form', icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>)},
    { name: 'Customer Directory', path: '/customers', icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg>)},
    { name: 'Order History', path: '/order-history', icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" /></svg>)},
    { name: 'Inventory', path: '/inventory', icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>)}
  ];

  // 3. Determine which sidebar to render (Removed /sales-entry from this check)
  const isOpsRoute = ['/delivery-form', '/customers', '/order-history', '/inventory'].some(path => location.pathname.startsWith(path));
  const currentNavItems = isOpsRoute ? opsItems : financeItems;
  const currentTitle = isOpsRoute ? 'Sales & Operations' : 'Financial Hub';
    ...
    <div className="h-screen overflow-hidden bg-brand-bg flex font-sans antialiased">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-72 bg-brand-dark flex flex-col justify-between h-screen transform transition-transform duration-300 ease-in-out shrink-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <div className="p-4 border-t border-white/5 shrink-0">
          ...
          {/* Only show Sign Out if the user is logged in (to prevent confusing ops workers on public devices) */}
          {auth.currentUser && (
            <button 
