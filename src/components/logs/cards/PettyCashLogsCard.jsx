...
export default function PettyCashLogsCard({ pettyCashData, loading, isExpanded, onExpand }) {
  ...
  const filteredData = useMemo(() => {
    ...
    if (!searchQuery.trim()) return pettyCashData;

    const lowerQuery = searchQuery.toLowerCase();
    return pettyCashData.filter(trx => {
      // Check description
        ...
        {loading ? (
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100/80 text-sm">
              {/* Check against filteredData instead of pettyCashData */}
                ...
                <tr>
                  <td colSpan="5" className="px-8 py-12 text-center text-slate-400 font-medium bg-slate-50/30">
                    <div className="flex flex-col items-center justify-center space-y-2">
                      ...
                      <span>{searchQuery ? 'No matching records found.' : 'No petty cash records found for this period.'}</span>
                    </div>
                  </td>
                </tr>
              ) : (
