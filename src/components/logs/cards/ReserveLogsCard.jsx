...
export default function ReserveLogsCard({ logs = [], loading = false }) {
      ...
      <div className="flex-1 overflow-y-auto min-h-[300px] max-h-[450px] p-2 space-y-1 bg-white custom-scrollbar">
        {loading ? (
          ...
          logs.map((log) => {
            ...
            return (
              <div key={log.id} className="grid grid-cols-12 gap-3 px-4 py-3.5 items-center hover:bg-indigo-50/40 rounded-xl transition-all duration-200 border border-transparent hover:border-indigo-100/50 group">
                
                {/* Date & Time */}
                <div className="col-span-3 flex flex-col">
                  <span className="text-[11px] font-semibold text-slate-700">{formatDate(log.createdAt)}</span>
                ...
                <div className="col-span-5 flex items-start gap-2 pl-2">
                  ...
                  <span className="text-xs font-medium text-slate-600 leading-snug line-clamp-2 pr-2" title={log.description}>
                    {log.description}
                  </span>
                </div>
                ...
                <div className="col-span-2 text-right">
                  <span className={`text-xs font-bold tracking-tight ${isZero ? 'text-slate-500' : isAdd ? 'text-emerald-500' : 'text-rose-500'}`}>
