...
export default function TransferLogsCard({ transfersData, loading, isExpanded, onExpand }) {
  ...
  const filteredData = useMemo(() => {
    ...
    if (!searchQuery.trim()) return transfersData;

    const lowerQuery = searchQuery.toLowerCase();
    return transfersData.filter(trx => {
      // Check description
   ...
   <div className={`bg-white rounded-[2rem] border border-slate-100/60 shadow-[0_8px_30px_rgb(0,0,0,0.03)] overflow-hidden animate-in slide-in-from-bottom-4 duration-500 flex flex-col transition-all ${
      ...
      <div className="bg-blue-500 px-8 py-5 flex items-center justify-between relative overflow-hidden shrink-0">
        ...
        <div className="relative z-10 flex items-center gap-3">
          ...
          <div>
            <h3 className="text-white font-bold text-lg tracking-wide uppercase">Transfers</h3>
      ...
      >
        {loading ? (
          <div className="h-full flex flex-col items-center justify-center space-y-4">
            ...
            <p className="text-slate-400 font-medium tracking-wide animate-pulse">Loading transfers...</p>
          </div>
        ) : (
                ...
                <tr>
                  <td colSpan="5" className="px-8 py-12 text-center text-slate-400 font-medium bg-slate-50/30">
                    <div className="flex flex-col items-center justify-center space-y-2">
                      ...
                      <span>{searchQuery ? 'No matching records found.' : 'No transfers or exchanges found for this period.'}</span>
                    </div>
                  </td>
                </tr>
              ) : (
