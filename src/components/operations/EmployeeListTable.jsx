...
const ExpiryCell = ({ date, idNumber }) => {
  ...
  if (!details) return <span className="text-slate-300 text-xs">-</span>;
  ...
  return (
    <div className="flex flex-col gap-1.5 w-full max-w-[140px]">
      ...
      <div className="flex justify-between items-end">
        <span className="text-slate-900 font-medium text-[13px] tracking-tight">{details.dateFormatted}</span>
      ...
      {idNumber && (
        <div className="flex items-center justify-between mt-0.5">
          <span className="text-[10px] font-medium text-slate-400 font-mono tracking-widest transition-all">
            {isRevealed ? idNumber : getMaskedId(idNumber)}
          </span>
          <button 
            onClick={() => setIsRevealed(!isRevealed)}
            className="text-slate-300 hover:text-brand-dark transition-colors p-0.5 rounded focus:outline-none"
...
export default function EmployeeListTable({ employees }) {
  ...
  return (
    <div className="bg-white rounded-[2rem] border border-slate-100/60 shadow-[0_8px_30px_rgb(0,0,0,0.03)] overflow-hidden flex flex-col">
      <div className="p-8 border-b border-slate-100/80 flex justify-between items-center bg-white relative z-10">
        <div>
          <h3 className="text-lg font-semibold text-slate-900 tracking-tight">Employee Directory</h3>
          <p className="text-xs text-slate-400 mt-1 font-light">Live tracking of personnel documents and compliance.</p>
        </div>
        <div className="flex gap-2">
          {/* ✅ UPDATED: Bound search input */}
          <input 
            type="text" 
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search employees..." 
            className="bg-slate-50 border border-slate-100 text-sm font-medium text-slate-600 rounded-xl px-4 py-2 focus:ring-2 focus:ring-brand-light/20 outline-none w-64 transition-all"
          />
          <button className="bg-slate-50 border border-slate-100 p-2 rounded-xl text-slate-500 hover:text-brand-dark hover:bg-slate-100 transition-colors">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50/50 text-slate-400 text-[10px] font-semibold uppercase tracking-widest">
              <th className="px-8 py-4 whitespace-nowrap">Employee Details</th>
              <th className="px-8 py-4 whitespace-nowrap">Emirates ID</th>
              <th className="px-8 py-4 whitespace-nowrap">Work Permit</th>
              <th className="px-8 py-4 whitespace-nowrap">Passport</th>
              <th className="px-8 py-4 text-right whitespace-nowrap">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100/80 text-sm">
            {/* ✅ UPDATED: Map over filtered array instead of full list */}
            {filteredEmployees.length === 0 ? (
              <tr>
                <td colSpan="5" className="px-8 py-12 text-center text-slate-400 font-medium">No employees match your search.</td>
              </tr>
            ) : (
                ...
                <tr key={index} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-8 py-5">
                    <div className="flex flex-col">
                      <span className="text-slate-900 font-semibold tracking-tight uppercase text-[13px]">
                        {emp.name}
                      </span>
                  ...
                  <td className="px-8 py-5 text-right whitespace-nowrap align-middle">
                    <button className="text-slate-400 hover:text-brand-dark font-medium text-sm transition-colors px-4 py-2 rounded-xl border border-transparent hover:border-slate-200 hover:bg-white hover:shadow-sm">
                      Manage
                    </button>
                  </td>
